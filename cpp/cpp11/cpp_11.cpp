...
int main() {
    ...
    // 3. Отсортировать по убыванию по цене
    ...
    // 5. Найти товар с ценой ниже заданного значения (например, ниже 50)
    auto it = std::find_if(products.begin(), products.end(), [](const Product& p){
        return p.getPrice() < 50;
    });
    
    if (it != products.end()) {
        std::cout << "\nНайден товар с ценой ниже 50:\n";
        it->display();
    } else {
        std::cout << "\nТовары с ценой ниже 50 не найдены.\n";
    }

    // 6. Переместить элементы, удовлетворяющие условию (например, цена < 50), в другой контейнер
    std::vector<Product> filteredProducts;
    
    // Используем copy_if для перемещения подходящих элементов
    std::copy_if(products.begin(), products.end(), std::back_inserter(filteredProducts),
                 [](const Product& p){ return p.getPrice() < 50; });
    
    // Можно удалить эти элементы из исходного контейнера (по желанию)
    // Для этого используем remove_if и erase
    ...
    // 8. Отсортировать оба контейнера по возрастанию цены
    auto sortByPriceAsc = [](std::vector<Product>& cont){
        std::sort(cont.begin(), cont.end(), [](const Product& a, const Product& b){
            return a.getPrice() < b.getPrice();
        });
    };
